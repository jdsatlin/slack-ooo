...
const STATUS_DEREGISTERED = 'deregistered';
...
class OOOUser {
  ...
  constructor (username, user) {
    if (user) {
      ...
      this.status = JSON.parse(user.status);
      this.message = JSON.parse(user.message);
      this.ooo_start = JSON.parse(user.ooostart);
      this.ooo_end = JSON.parse(user.oooend);
    } else {
  ...
  setMessage (message) {
    ...
    dbwrite.updateMessage(this, message);
  ...
  setStart (start) {
    ...
    if (time.isValid()) {
      this.ooo_start = time;
      dbwrite.updateUserStart(this, time);
  ...
  setEnd (end) {
    ...
    if (time.isValid()) {
      this.ooo_end = time;
      dbwrite.updateUserEnd(this, time);
      if (time.isBefore()) {
        retVal = 'You are no longer marked Out of Office';
        this.status = STATUS_UNCONFIRMED;
        dbwrite.updateStatus(this);
      } else {
        if (!this.ooo_start) {
      ...
      switch (this.status) {
        ...
        case STATUS_AWAITING_MESSAGE:
          if (this.lastCommunication() < TIMEOUT_MESSAGE) {
            this.status = STATUS_REGISTERED;
            dbwrite.updateStatus(this, this);
        ...
        case STATUS_AWAITING_DEREGISTER:
          if (this.lastCommunication() < TIMEOUT_MESSAGE) {
            if (message.match(POSITIVE_REGEXP)) {
              ...
              this.ooo_start = null;
              this.ooo_end = null;
              dbwrite.updateUserStart(this, null);
              dbwrite.updateUserEnd(this, null);
              dbwrite.updateStatus(this);
...
OOOUser.STATUS_DEREGISTERED = STATUS_DEREGISTERED;
