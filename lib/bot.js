...
const dbread = require('./dbread.js');
...
class Bot {
  ...
  constructor (config) {
    ...
    this.ooo_users =  depromisify(dbread.loadUsers()) //new Map();  //
  ...
  handleDirectCommand (message) {
    // Start a new user
    console.log(this.ooo_users)
  //  console.log(this)
    if (!this.ooo_users.get(message.user)) {
      this.ooo_users.set(message.user, new OOOUser(message.user));
      let currentUser = this.ooo_users.get(message.user);
      dbwrite.createUser(currentUser);
    }
